                      ...
                      <div className="min-w-0 flex-1 space-y-1">
                        ...
                        <div className="flex justify-between items-center text-[10px]">
                          <div className="flex items-center gap-1.5 min-w-0">
                            ...
                            <LeadQueueBadge lead={lead} asesores={asesores} />
                          </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTES DE FEEDBACK VISUAL DE ROTACIÓN DE LEADS
// ─────────────────────────────────────────────────────────────────────────────
function LeadQueueBadge({ lead, asesores }: { lead: Lead; asesores: { id: string; name: string }[] }) {
  const [timeLeft, setTimeLeft] = useState<number | null>(null);

  useEffect(() => {
    if (lead.estado_asignacion !== "en_cola" || !lead.inicio_turno) return;

    const calc = () => {
      const start = new Date(lead.inicio_turno!).getTime();
      const elapsed = Math.floor((Date.now() - start) / 1000);
      const remaining = (lead.timeout_nivel || 30) - elapsed;
  ...
  if (lead.estado_asignacion !== "en_cola") return null;

  const currentCandidateName = lead.candidato_actual
    ? (asesores.find((a) => a.id === lead.candidato_actual)?.name || "Asesora")
    : null;

  const phaseLabel =
    lead.golden_ticket_phase === "individual"
      ? `Cola: ${currentCandidateName || "Asesora"}`
      : lead.golden_ticket_phase === "tier_expanded"
      ? "Cola: Grupo"
  ...
  return (
    <span className="text-[8px] font-black text-amber-700 bg-amber-50 border border-amber-200 px-1.5 py-0.2 rounded shrink-0 animate-pulse flex items-center gap-1">
      <span className="h-1.5 w-1.5 rounded-full bg-amber-500 animate-ping" />
      {phaseLabel} {timeLeft !== null ? `(${timeLeft}s)` : ""}
    </span>
  );
}

function LeadQueuePaneDetail({ lead, asesores }: { lead: Lead | null; asesores: { id: string; name: string }[] }) {
  const [timeLeft, setTimeLeft] = useState<number | null>(null);

  useEffect(() => {
    if (!lead || lead.estado_asignacion !== "en_cola" || !lead.inicio_turno) return;
  ...
  if (!lead || lead.estado_asignacion !== "en_cola") return null;

  const currentCandidateName = lead.candidato_actual
    ? (asesores.find((a) => a.id === lead.candidato_actual)?.name || "una asesora")
    : "";

  return (
    <div className="bg-amber-50 border-b border-amber-200/85 px-4 py-2.5 flex items-center justify-between text-xs text-amber-800 shrink-0">
      <div className="flex items-center gap-2.5">
        <span className="text-base select-none">🎫</span>
        <div>
          <span className="font-extrabold uppercase tracking-wider text-[10px] text-amber-700 block">Asignación Golden Ticket (En Cola)</span>
          <p className="text-[11px] text-amber-900 font-medium leading-tight">
            {lead.golden_ticket_phase === "individual" ? (
              <>Asignado en exclusividad a <strong>{currentCandidateName}</strong>. Si no acepta en {timeLeft}s, pasará al nivel/grupo.</>
            ) : lead.golden_ticket_phase === "tier_expanded" ? (
              <>Fase grupal: Disponible para cualquier asesora de su Nivel/Tier. Quedan {timeLeft}s.</>
            ) : (
              <>Fase de Rescate: Abierto de forma urgente a cualquier asesora de Nivel 1. Quedan {timeLeft}s.</>
            )}
          </p>
        </div>
      </div>
      <div className="text-[10px] bg-amber-500 text-white font-black px-2.5 py-1 rounded-full shrink-0 shadow-xs flex items-center gap-1 select-none">
  ...
  return (
    <div className="flex-1 flex overflow-hidden h-full relative">
      ...
      <div className="flex-1 flex flex-col overflow-hidden h-full">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex bg-gray-150/65 p-0.5 rounded-lg border border-gray-200/50 text-[10px]">
              ...
              <button
                ...
                disabled={togglingChatbot}
                className={`px-2 py-1 font-black rounded flex items-center gap-0.5 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-wait ${
                  !lead?.chatbot_activo ? "bg-green-600 text-white shadow-2xs" : "text-gray-500 hover:text-gray-800"
                }`}
              >
                👤 Humano
              </button>
        ...
        {/* Rotation Feedback Banner for Admin / Advisors */}
        <LeadQueuePaneDetail lead={lead} asesores={asesores} />
