...
import { FiSearch, FiEdit2, FiTrash2, FiSave, FiX, FiPlus, FiUsers, FiPhone, FiMapPin, FiMap, FiClock, FiInfo, FiTruck, FiClipboard, FiChevronUp } from 'react-icons/fi';
...
function ClienteFormFields({ form, setForm }: { form: ClienteForm; setForm: React.Dispatch<React.SetStateAction<ClienteForm>> }) {
...
export default function ClientesPage() {
  ...
  const [createForm, setCreateForm] = useState<ClienteForm>({ distrito: 'La Victoria', tipo_cliente: 'Frecuente', empresa: 'Transavic' });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCreateForm({ distrito: 'La Victoria', tipo_cliente: 'Frecuente', empresa: 'Transavic' });
      } else {
  ...
  const toggleHistory = async (clienteId: string) => {
    ...
    if (historyPedidos[clienteId]) return; // already cached
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm">Gestiona tu base de datos de clientes. Los datos guardados aquí se auto-llenarán al crear pedidos.</p>
      </div>
      ...
      {showCreateForm && (
        <div className="mb-6 bg-white border-2 border-red-200 rounded-xl shadow-md p-5">
          ...
          <ClienteFormFields form={createForm} setForm={setCreateForm} />
          <div className="flex gap-2 mt-4">
            <button onClick={handleCreate} disabled={creating} className="flex items-center gap-2 px-5 py-2.5 bg-green-500 text-white font-semibold rounded-lg hover:bg-green-600 transition-colors text-sm disabled:bg-gray-300"><FiSave size={14} />{creating ? 'Guardando...' : 'Guardar Cliente'}</button>
            <button onClick={() => { setShowCreateForm(false); setCreateForm({ distrito: 'La Victoria', tipo_cliente: 'Frecuente', empresa: 'Transavic' }); }} className="flex items-center gap-2 px-5 py-2.5 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition-colors text-sm"><FiX size={14} />Cancelar</button>
          </div>
        </div>
      )}
            ...
            <div key={c.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
              {editingId === c.id ? (
                <div>
                  ...
                  <ClienteFormFields form={editForm} setForm={setEditForm} />
                  <div className="flex gap-2 mt-4">
                    <button onClick={saveEdit} disabled={saving} className="flex items-center gap-2 px-5 py-2.5 bg-green-500 text-white font-semibold rounded-lg hover:bg-green-600 transition-colors text-sm disabled:bg-gray-300"><FiSave size={14} />{saving ? 'Guardando...' : 'Guardar'}</button>
                    <button onClick={cancelEdit} className="flex items-center gap-2 px-5 py-2.5 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition-colors text-sm"><FiX size={14} />Cancelar</button>
                  </div>
                </div>
              ) : (
                <>
                <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    ...
                    <button onClick={() => startEdit(c)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="Editar"><FiEdit2 size={16} /></button>
                    <button onClick={() => handleDelete(c.id, c.nombre)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar"><FiTrash2 size={16} /></button>
                  </div>
                </div>
                {/* Order History Panel */}
