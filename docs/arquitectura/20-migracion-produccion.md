...
```bash
psql "$DATABASE_URL_UNPOOLED" -v ON_ERROR_STOP=1 -f scripts/migrate-<feature>.sql
```
...
| 12 jul 2026 | ⏳ solo `dev-hugo` | facturación de Campo, claims/NC, vistas generales y corrección auditada de CPE rechazados |
...
test -n "$DB_DEV_URL"
psql "$DB_DEV_URL" -v ON_ERROR_STOP=1 -f scripts/migrate-<feature>.sql
unset DB_DEV_URL
```
...
## 4. Pase pendiente: facturación de Campo

Orden actual previsto:
...
Verificación exacta y casos de doble pestaña: [24 §3–6](./24-pruebas-regresion-despliegue.md).
