...
#### Runbook: crear las credenciales en el portal SOL (19 ago 2026 — ejecutado y verificado con el RUC 10)

Fuente: [Manual oficial de Consulta Integrada de CPE por Servicio Web v2.0](https://cpe.sunat.gob.pe/sites/default/files/inline-files/Manual-de-Consulta-Integrada-de-Comprobante-de-Pago-por-ServicioWEB_v2_0.pdf) (SUNAT) + ejecución real del 19/08/2026 (Avícola). Se repite **por cada RUC** (Transavic 20612806901 pendiente; Avícola de Tony 10710548841 ✅), con la clave SOL de ese RUC.

1. Entrar a **SUNAT Operaciones en Línea** (`https://e-menu.sunat.gob.pe/cl-ti-itmenu/MenuInternet.htm`) e iniciar sesión con **RUC + usuario SOL + contraseña**.
2. Navegar el menú: **Empresas → Comprobantes de pago → Consulta de Validez de Comprobante de Pago → Consulta de Validez de Comprobante de Pago → Credenciales de API SUNAT**. (En el RUC 10 el bloque aparece bajo "Personas"; mismo camino desde "Comprobantes de pago".)
   - ⚠️ **NO usar** la entrada de primer nivel **"Credenciales de API SUNAT" → "Gestión Credenciales de API SUNAT"** (la del menú raíz): esa es la pantalla UNIFICADA con checkboxes por API donde vive la app de **GRE** (en el RUC 10: "TRANSAVIC ERP", id `1fcf156c-…`, con "GRE Emisión de Comprobantes" marcada). **No tocar esa app** — ni sus checkboxes ni su 🔄. La app de Consulta se registra en el camino de Comprobantes de pago, que es un registro separado (nace vacío aunque exista la de GRE).
3. La primera vez el sistema pregunta **"¿DESEA HABILITAR EL CONSUMO DEL API SUNAT?"** → botón **HABILITAR**. Sale un **diálogo nativo de confirmación** del navegador → Aceptar. (Si el RUC ya estaba habilitado, va directo al registro.)
4. En **"REGISTRE SU APLICACIÓN"** llenar:
   - **Nombre de su aplicación**: p. ej. `AVICOLA ERP CONSULTA CPE` (texto libre, solo identifica la app).
   - **URL de su aplicación**: `https://app.transavic.com` (informativa; `client_credentials` no usa callback).
   - Botón **REGISTRAR** → modal "La aplicación se registró correctamente".
5. La app aparece en **"APLICACIONES REGISTRADAS"** con dos valores visibles: **Id** (= `client_id`, formato UUID) y **Clave** (= `client_secret`, base64 de 24 chars). Copiarlos de inmediato a `CREDENCIALES-PRODUCCION.local.md` y `.env.local` (las env de Vercel nacen Sensitive y no se releen).
6. ⚠️ El botón **🔄** de la lista **REGENERA** la credencial (invalida la anterior). No tocarlo después de configurar Vercel, o la consulta vuelve a fallar (401) hasta actualizar las env vars.
7. **Validar ANTES de subir a Vercel** (lección del doc 28 con los tokens de Meta): pedir un token con `curl` al endpoint de abajo → debe responder `access_token` con `expires_in: 3600`. El 19/08 funcionó al instante (sin la espera de propagación de 15 min–2 h que sí tuvieron las credenciales GRE en 2026-06).
8. Cargar en Vercel: `SUNAT_AVI_CONSULTA_CLIENT_ID`/`_SECRET` (RUC 10) y `SUNAT_TRA_CONSULTA_CLIENT_ID`/`_SECRET` (RUC 20) + redeploy (las env solo aplican al próximo build).

Verificación sin tocar el ERP (opcional, con `curl`/Postman): `POST https://api-seguridad.sunat.gob.pe/v1/clientesextranet/{client_id}/oauth2/token/` con body `x-www-form-urlencoded`: `grant_type=client_credentials`, `scope=https://api.sunat.gob.pe/v1/contribuyente/contribuyentes`, `client_id`, `client_secret` → debe devolver `access_token` (`expires_in` 3600). Luego `POST https://api.sunat.gob.pe/v1/contribuyente/contribuyentes/{ruc}/validarcomprobante` con `Authorization: Bearer …` y JSON `{numRuc, codComp:"03", numeroSerie:"B002", numero, fechaEmision:"dd/mm/yyyy", monto}`. `estadoCp`: `0`=NO EXISTE, `1`=ACEPTADO, `2`=ANULADO (comunicado en una baja); `3`/`4` son de comprobantes físicos de imprenta.

**Límite de ambiente:** ambos mecanismos de consulta se usan solo en producción. En local/BETA con `dev-hugo` no se consulta ningún endpoint productivo ni se cruzan RUC, correlativos o estados. La migración/queries SQL se validan en `dev-hugo`; los contratos SOAP 01 y REST 03 se prueban con mocks mediante `npm run test:reconciliacion-sunat`. No existe E2E de consulta en BETA. Las credenciales de **Avícola** (`SUNAT_AVI_CONSULTA_*`) se crearon y validaron el 19/08/2026 (token + `validarcomprobante` reales OK); las de **Transavic** (`SUNAT_TRA_CONSULTA_*`) siguen pendientes — sus boletas B001 chocarían con el mismo `CFG_API03`.
