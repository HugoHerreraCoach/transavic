...
23. **Imprimir SIEMPRE necesita `@page { size }` o la tiquetera saca enormes espacios en blanco (fix 4 jun 2026)**: el **"REPORTE DE PEDIDOS"** (`src/components/VistaImpresion.tsx`, el reporte de TODOS los pedidos del día que se imprime desde el Dashboard, NO confundir con la "orden de pedido" individual `guia-imprimible-client.tsx`) salía en la tiquetera térmica con **grandes tramos de papel en blanco entre pedidos**. Causa: el componente **no definía ningún `@page`**, así que el navegador paginaba con el **tamaño por defecto (A4, altura fija ~297mm)**; como cada tarjeta de pedido lleva `break-inside: avoid` / `page-break-inside: avoid` (en `globals.css`), cuando un pedido no entraba en lo que quedaba de la "hoja A4 virtual", el navegador lo empujaba a la siguiente y **dejaba vacío el resto de la hoja** → en el rollo continuo de la térmica eso es un hueco enorme (aparecía justo después de los pedidos largos). **Fix**: se inyecta `@page { size: 80mm auto; margin: 0 }` cuando el formato es **Ticket** (y `size: A4; margin: 1cm` cuando es A4) vía un `<style dangerouslySetInnerHTML>` en `VistaImpresion.tsx`. El `auto` en la altura = **papel continuo** (sin páginas de altura fija) → los `break-inside:avoid` dejan de generar huecos. Es el MISMO patrón que ya usaba con éxito la orden individual (`guia-imprimible-client.tsx:159` con `@page size: ${esTicket ? "80mm auto" : "A4"}`). **Regla general: cualquier vista pensada para tiquetera 80mm DEBE declarar `@page { size: 80mm auto }`**; si no, hereda el A4 del navegador y desperdicia papel. (El CSS de impresión solo se valida 100% imprimiendo; tsc/eslint no lo cubren.)
...
### Boleta a NOMBRE del cliente SIN DNI (4 jun 2026 — ✅ local + verificado en BETA · ⏳ falta merge/deploy a prod)
...
- **P0 "Cierra el loop del dinero" (mayo 2026 — brainstorming → spec → plan → ejecutado)**: audit completo en `docs/superpowers/specs/2026-05-27-audit-conexiones-roadmap-design.md` + plan task-by-task en `docs/superpowers/plans/2026-05-27-p0-cierra-loop-dinero.md`. Implementado:
  - **Factura Contado → cobranza por default** + checkbox "Ya cobrado" para opt-out (cash de mano). Refleja la realidad Transavic ("contado = paga después" en la mayoría de casos). Boletas al CONTADO no crean cobranza (consumidor cash); **una boleta a CRÉDITO sí crea cobranza** (es venta por cobrar — lo cubre `esCredito`, igual que la factura). `emit-manual/route.ts` + `emit-client.tsx`.
...
- **P0.1 — Contado → cobranza por default** (`/api/comprobantes/emitir-manual` + `emitir-client.tsx`): toda factura (tipo 01) crea cobranza automáticamente, sea Contado o Crédito. Toggle "El cliente ya pagó al instante" cuando es Contado-cash. Boletas (03) al contado no crean cobranza; **a crédito sí** (venta por cobrar).
