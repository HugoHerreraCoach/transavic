...
**Producción (Vercel) tiene configuradas las variables de emisión/GRE del lanzamiento** (`SUNAT_ENVIRONMENT=production`, usuarios SOL y certificados), además de DB, Auth, Maps, correo, IA y `CRON_SECRET`. **Excepción:** todavía faltan las cuatro `SUNAT_*_CONSULTA_CLIENT_*` de boletas; no afirmar que están “todas” hasta crearlas por separado para ambos RUC. Las credenciales reales viven solo en Vercel y archivos locales ignorados por Git.
...
71. **Boletas `por_confirmar` de Avícola: el bloqueo es correcto, falta la llave (11 ago 2026)**: 9 boletas atascadas desde el 15 jul porque SUNAT falló al recibirlas (`0130`/`0133`/`0140` — fallas de SUNAT, no rechazos) y **las 4 `SUNAT_*_CONSULTA_CLIENT_*` nunca se crearon**, así que la consulta REST de boletas devuelve `CFG_API03` y reprograma +6 h para siempre (B002-225: **75 intentos**). No se pueden anular porque la baja de una boleta va por el RC con `estadoItem=3` (ver #70). **No re-emitir**: las 4 anteriores al 20 jul ya tienen reemplazo aceptado (posible duplicado fiscal → NC); las 5 posteriores están limpias gracias al candado. Se destraba creando la app **"Consulta de Validez de Comprobantes"** en SOL por RUC (OAuth `client_credentials`, `clientesextranet`, scope `contribuyente/contribuyentes`) — **NO reutilizar ni rotar las `SUNAT_*_CLIENT_ID/SECRET` de GRE**. Detalle: [historial](./docs/historial-cambios-2026.md).
