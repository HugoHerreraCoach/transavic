...
- **Capacitor 7** (`capacitor.config.ts`: `appId pe.transavic.reparto`, `useLegacyBridge:true`, `server.url` configurable por `CAP_SERVER_URL`, default `http://localhost:3000`). Proyecto en `android/`. **APK debug compila OK** (`android/app/build/outputs/apk/debug/app-debug.apk`, ~4.4MB). Scripts: `npm run app:sync|app:open|app:build|app:build:prod`.
...
- **Pendiente (del usuario):** la **prueba de calle en teléfono real** (pantalla apagada 30–60 min) es lo único que falta; recién al pasar sale el deploy + APK de producción + Play Internal Testing.
