...
44. **Compras: 3 clases de fila y deuda manual (9-10 jul 2026, pedidos de Nelita)**: cada fila de una guía de compra es `ingreso` (default), `devolucion` (RESTA deuda e inventario — kardex `devolucion_compra`, subtotal negativo pero pesos SIEMPRE positivos: el signo vive en `compra_items.tipo`) o **servicio** (categoría del producto matchea `/servicio/i`, ej. "Pelada de pollo"/"ENVIO": cantidad × precio, suma a la deuda pero JAMÁS toca stock/kardex/`precio_compra`). El total de la guía nunca queda < 0 (400) y con total 0 no se crea cuenta por pagar. **Saldo anterior de proveedor** = fila de `cuentas_por_pagar` con `compra_id NULL` + `concepto` (botón "Deuda anterior" en CxP, admin-only); se paga con el flujo normal; DELETE/PATCH solo para manuales sin pagos. Migración `migrate-compras-mejoras-2026-07-09.sql`. Detalle: [doc 09 §3.1b-3.2b](./docs/arquitectura/09-compras-inventario-mermas.md).
