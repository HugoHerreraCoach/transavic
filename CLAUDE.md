...
> **📚 Para profundizar en cualquier área:** ver `docs/arquitectura/` (27 documentos temáticos verificados contra código). Empezar por [`docs/arquitectura/README.md`](./docs/arquitectura/README.md) que tiene un mapa "si vas a tocar X, lee Y". Las **crónicas completas** de cada cambio (PRs, data-ops, diagnósticos) viven en [`docs/historial-cambios-2026.md`](./docs/historial-cambios-2026.md).
...
**Migraciones:** no hay runner automatizado. Los `.mjs` antiguos se conservan como
historial, pero toda migración nueva debe ser SQL idempotente y tener rollback:

```bash
psql "$DB_DEV_URL" -1 -v ON_ERROR_STOP=1 -f scripts/migrate-<feature>.sql
psql "$DB_DEV_URL" -1 -v ON_ERROR_STOP=1 -f scripts/rollback-<feature>.sql
```

Primero se prueba en `dev-hugo`. En producción se ejecuta con
`psql "$DATABASE_URL_UNPOOLED" -1 -v ON_ERROR_STOP=1 -f ...` **antes** del deploy del
código. No modificar migraciones existentes ni ejecutar una migración de
producción sin autorización explícita.

> **Migración a producción (30 may 2026):** el esquema de producción se puso al día con **`scripts/migrate-produccion-2026-05-29.sql`** (consolida 8 tablas + 14 columnas que faltaban; idempotente y aditivo). Se aplica con **psql**, NO con los `.mjs` (Node 26 + `@neondatabase/serverless` falla — gotcha #13): `psql "$DATABASE_URL_UNPOOLED" -1 -v ON_ERROR_STOP=1 -f scripts/migrate-produccion-2026-05-29.sql`. Rollback: `scripts/rollback-produccion-2026-05-29.sql`. Para futuros cambios de esquema, aplicar a producción por psql **antes** de que el deploy del código nuevo quede activo.
...
| `WHATSAPP_TRA_*` / `WHATSAPP_AVI_*` (`PHONE_NUMBER_ID`, `TOKEN`, `WABA_ID`) + `WHATSAPP_API_VERSION` | Credenciales de la WhatsApp Cloud API **por marca** (TRA=Transavic RUC 20, AVI=Avícola de Tony RUC 10), mismo patrón per-empresa que `SUNAT_*`. El webhook rutea el entrante a la marca por `metadata.phone_number_id`. Sin las credenciales de una marca, esa marca queda en **modo mock** (registra en el CRM, no manda a Meta) — no rompe nada. Ver gotcha #52 y [doc 15 §5](./docs/arquitectura/15-asistente-ia.md). |
| `BREVO_API_KEY`, `BREVO_SENDER_EMAIL`, `BREVO_SENDER_NAME` | Brevo (correos transaccionales, free **300/día POR CUENTA** — las 2 marcas comparten ese cupo). Si `BREVO_API_KEY` está, `lib/email.ts` usa la API de Brevo (preferida); si no, cae a SMTP/nodemailer. El sender debe estar verificado en Brevo (hoy `transavicdev@gmail.com`, activo). **Configurar también en Vercel.** |
| `BREVO_TRA_SENDER_EMAIL`/`_NAME`, `BREVO_AVI_SENDER_EMAIL`/`_NAME` | **Remitente POR MARCA** (TRA=Transavic `@transavic.com`, AVI=Avícola de Tony `@laavicoladetony.com`), mismo patrón per-empresa que `SUNAT_*`. Fuente única: `resolverRemitente()` en `lib/brevo.ts` (la usan la rama Brevo y la SMTP). **Opcionales:** sin ellas cae a `BREVO_SENDER_*` y todo se comporta como antes. Ver gotcha #53. |
| `CRON_SECRET` | Secreto que protege los **6 cron jobs** de Vercel (`facturas-vencidas`, `recordatorios-asesoras`, `resumen-diario-sunat`, `daily-digest-admin`, `repartidores-oscuros`, `reconciliar-cpe-sunat`). Sin él devuelven **503**. Vercel lo manda como `Authorization: Bearer <CRON_SECRET>`. **Obligatorio en Vercel.** Repartidores oscuros corre cada 10 min y reconciliación CPE cada 5 min. Vercel Pro permite hasta 40 crons. |
...
| `SUNAT_TRA_CONSULTA_CLIENT_ID`, `SUNAT_TRA_CONSULTA_CLIENT_SECRET`, `SUNAT_AVI_CONSULTA_CLIENT_ID`, `SUNAT_AVI_CONSULTA_CLIENT_SECRET` | OAuth separado de **Consulta Integrada de Comprobantes de Pago** para boletas `03` de cada RUC. No son las credenciales GRE `SUNAT_*_CLIENT_ID/SECRET` y nunca deben reutilizarlas o sustituirlas. **Las `AVI` existen desde el 19 ago 2026** (Vercel + `.env.local`; runbook en doc 11); las `TRA` siguen pendientes. Sin la credencial de su empresa, una boleta ambigua queda `por_confirmar` + revisión y bloquea duplicados (gotcha #71). |
...
**Producción (Vercel) tiene configuradas las variables de emisión/GRE del lanzamiento** (`SUNAT_ENVIRONMENT=production`, usuarios SOL y certificados), además de DB, Auth, Maps, correo, IA y `CRON_SECRET`. **Excepción:** de las cuatro `SUNAT_*_CONSULTA_CLIENT_*` de boletas, las `AVI` ya están (19 ago 2026); faltan las dos `TRA` — no afirmar que están "todas" hasta crearlas para el RUC 20. Las credenciales reales viven solo en Vercel y archivos locales ignorados por Git.
...
13. **Bug DNS Node 26 con `@neondatabase/serverless`**: scripts `node ./scripts/migrate-X.mjs` fallan con `TypeError: fetch failed`. Workaround: aplicar SQL directamente con `psql "$DATABASE_URL_UNPOOLED" -1 -v ON_ERROR_STOP=1 -f scripts/migrations-fase-ab.sql`. Next.js dev server NO está afectado (usa su propio runtime). Nota: `npm install` SÍ funciona (verificado mayo 2026).
...
17. **Producción lanzada 30 may 2026 — cómo se migró**: el esquema de producción se llevó al día con `scripts/migrate-produccion-2026-05-29.sql` aplicado por **psql** (`psql "$DATABASE_URL_UNPOOLED" -1 -v ON_ERROR_STOP=1 -f …`), NO por los `.mjs` (Node 26 los rompe — gotcha #13). Rollback en `scripts/rollback-produccion-2026-05-29.sql`. Vercel: proyecto `hugoherrerateam/transavic`, plan **Pro**. Para futuros cambios de esquema: probar en `dev-hugo`, y al mergear a `main` aplicar la migración a producción por psql ANTES de que el deploy con el código nuevo quede activo (si no, el código nuevo choca con columnas/tablas que faltan).
...
44. **Compras: 3 clases de fila y deuda manual (9-10 jul 2026, pedidos de Nelita)**: cada fila de una guía de compra es `ingreso` (default), `devolucion` (RESTA deuda e inventario — kardex `devolucion_compra`, subtotal negativo pero pesos SIEMPRE positivos: el signo vive en `compra_items.tipo`) o **línea SIN peso** (cantidad × precio, suma a la deuda pero JAMÁS toca stock/kardex/`precio_compra`). La regla de "sin peso" vive en **`src/lib/compras-lineas.ts:esLineaSinPeso`** (fuente ÚNICA compartida front+back, antes duplicada como `esCategoriaServicio`): la categoría matchea **`/servicio|insumo|adicional/i`** — cubre Servicios (Pelada/ENVIO), **Insumos** (arcos/oferta/mandil — 11 jul 2026) y el genérico "producto adicional". El total de la guía nunca queda < 0 (400) y con total 0 no se crea cuenta por pagar. **Nuevo producto sin salir de Compras**: botón "➕ Nuevo producto" (solo admin) en `compras-client.tsx` → `POST /api/productos` (admin-only) → append + auto-selección; categoría "Insumos" → prefijo de código `INS`. **Saldo anterior de proveedor** = fila de `cuentas_por_pagar` con `compra_id NULL` + `concepto` (botón "Deuda anterior" en CxP, admin-only); se paga con el flujo normal. **DELETE** solo para manuales sin pagos; el **PATCH** (16 jul 2026) SÍ corrige una manual **aunque ya tenga pagos** — la cascada FIFO se lo aplica primero por ser el vencimiento más antiguo y antes quedaba imposible de corregir: si el monto nuevo es menor a lo aplicado, el excedente se despega y queda como **saldo a favor** (anticipo) del proveedor. Va en `sql.transaction` con `consultaBloqueoProveedor()` primero, recorta el cache `monto_pagado` con `LEAST` en el MISMO UPDATE (el CHECK `monto_pagado <= monto_deuda + 0.01` es por fila) y recalcula desde las aplicaciones. Botón ✏️ en la ficha del proveedor y en CxP; las deudas de compra siguen intocables. Migración `migrate-compras-mejoras-2026-07-09.sql` + seed `seed-insumos-compras-2026-07-11.sql`. Detalle: [doc 09 §3.1b-3.2b](./docs/arquitectura/09-compras-inventario-mermas.md).
...
48. **POS de planta: catálogo "Principales" + panel "Ventas de hoy" (13 jul 2026 — pedido de Ariana)**: (a) el catálogo del POS abre en la pestaña **"Principales"** (pollo entero/brasa, carcasa, espinazo, molleja, patas de pollo, menudencia mixta, alas) para no verse cargado de carnes que no se venden de madrugada; el resto va en las otras categorías o en la **búsqueda (que ahora MANDA sobre la categoría** — busca en todo el catálogo). Lista FIJA en `PRINCIPALES_PATRONES` de `pos-client.tsx` (ajustable; matcher por nombre acotado para no capturar res/cerdo). (b) Panel colapsable **"Ventas de hoy"** en el POS (`GET /api/pos/resumen-dia`): total del día, **DÓNDE CAYÓ EL DINERO** (por cuenta/caja del contado + por cobrar del crédito) y últimas ventas — responde el "no veo dónde se acumula el dinero" (el contado del POS suma a la cuenta elegida en "Cobrar en" → `cuentas_bancarias`+`transacciones`; el crédito → `cobranzas_planta`). **Gotcha de fecha:** el desglose por cuenta se filtra por la fecha del **PEDIDO** (`p.created_at`), NO por la de la `transaccion` — cerca de medianoche caían en días distintos y el total salía 0 con la venta igual visible en el historial. Sin migraciones (solo lectura). Detalle: [doc 10](./docs/arquitectura/10-pos-caja-tesoreria.md).
49. **POS: "Ventas de Planta" + ANULAR una venta (13 jul 2026 — pedido de Ariana)**: vista `/dashboard/pos-planta/ventas` (admin+produccion) lista el POS por Hoy/Ayer/Semana/fecha (`GET /api/pos/ventas`, espejo de avícola) y **anula** una venta (`POST /api/pos/ventas/[id]/anular`) reversando **dinero + stock**. **Reversión ATÓMICA en UNA sola `sql.transaction`**: el "claim" (`pedidos.anulada=TRUE`) es la PRIMERA sentencia de esa misma transacción, y si se pierde por carrera (doble-tap) `SELECT 1/(SELECT COUNT(*) FROM claim)` fuerza `division_by_zero` → **ROLLBACK total** (NO existe la ventana "anulada pero sin reversar"; NO hay claim-fuera + release manual). Efectos: `inventario_lotes += cantidad` + movimiento `anulacion_venta_pos`; por cada `ingreso`, `saldo -= monto` + **EGRESO compensatorio** en la MISMA cuenta (ingreso+egreso=0, reversa el monto REAL del ingreso, no recálculo de ítems); cobranza a crédito → `anulada=TRUE, estado='Anulada'`. **Guardas (409):** comprobante SUNAT vivo → Nota de Crédito; **caja de planta de ese día ya cerrada** (arqueada) si el cobro cayó en su cuenta → ajuste manual; venta a crédito **con abonos** → gestionar devolución. Las anuladas se **excluyen de TODOS los totales**: `resumenVentasGeneralesPorFecha` (Ventas Generales+Consolidado), `resumen-dia` y **`rentabilidad`** (filtra `estado='Entregado'` → ahora también `AND NOT anulada`). `Editar`=anular y rehacer (v1). Las anuladas se **OCULTAN de la lista por defecto** (toggle "Ver/Ocultar anuladas") para no hacer ruido visual — NO hay DELETE físico (los movimientos de dinero/inventario quedan como auditoría; "jamás DELETE" #41/#45). Migración `migrate-pos-anular-2026-07-13.sql` (campos `anulada*` en `pedidos`, aditiva; a prod por psql ANTES del deploy). Endurecido tras revisión adversarial; verificado E2E en beta y prod (reversión cuadra). Detalle: [doc 10 §2.4](./docs/arquitectura/10-pos-caja-tesoreria.md).
50. **Paquete operativo 13 jul 2026:** Ventas de Ejecutivas usa solo pedidos asesor conciliados y completamente pesados (`subtotal_real`), sin sumar CPE; pagos de proveedores se contabilizan por aplicaciones activas y excedentes como saldo a favor; POS congela costo del servidor en `costo_unitario_snapshot`; Producción solo reprograma a mañana con auditoría y alerta atómicas. Metas/comisiones quedan fuera. Detalle: docs 10, 16, 26 y 27.
51. **Usuario "solo lectura" — candado en el MIDDLEWARE (16 jul 2026)**: `users.solo_lectura BOOLEAN` (Fase 1 de roles flexibles). El candado NO está en cada ruta: como casi todas las mutaciones son rutas `/api` no-GET (solo hay 1 Server Action, login/logout), el `authorized` de `src/auth.config.ts` **rechaza con 403 cualquier método distinto de GET/HEAD/OPTIONS** cuando `auth.user.solo_lectura`. El matcher del middleware ahora cubre `/api` (antes lo excluía) **menos `/api/auth`** (login/logout intactos). **Implicancia para futuras rutas:** una ruta GET que MUTE datos se saltaría el candado — nunca escribir en un handler GET (los crons con CRON_SECRET son aparte, sin sesión → no los afecta). La bandera viaja en el JWT → **un cambio de la bandera aplica en el próximo login** del usuario. El observador de Antonio = rol `admin` + `solo_lectura=TRUE` (ve todo el alcance admin, no escribe nada). **Fase 2 (16 jul):** `users.vistas_permitidas TEXT[]` (NULL = sin restricción) limita QUÉ secciones ve/abre un usuario — el catálogo y la lógica del gate viven en **`src/lib/vistas.ts`** (`rutaPermitidaPorVistas`, `sanearVistas`, `primeraVistaPermitida`; keys = hrefs de `navItems`, mantener en sync). El sidebar filtra por esa lista y el `authorized` del middleware redirige (solo GET de `/dashboard`) a la primera vista permitida si la página no está permitida. **`vistas` solo NARROWA dentro del rol y es VISIBILIDAD, no frontera de datos** (el scope de datos lo sigue dando el rol en cada API). `[]`/basura → NULL (anti-lockout). Ambas banderas van en el JWT → aplican al próximo login. Toggles en el modal de usuarios (admin-only). Detalle: [historial](./docs/historial-cambios-2026.md).

52. **CRM WhatsApp real para DOS marcas (19 jul 2026)**: el CRM ya envía/recibe por **Meta Cloud API** (antes el saliente era mock). Reglas: (a) **un webhook COMPARTIDO** (`/api/webhooks/meta`) rutea el entrante a la marca por `value.metadata.phone_number_id` → `empresaDesdePhoneNumberId` en **`src/lib/whatsapp/config.ts`** (si NINGUNA marca tiene phone id configurado = entorno mock → cae a "Transavic"; si hay marcas pero el id no matchea = tráfico ajeno → se IGNORA); (b) credenciales **por marca** `WHATSAPP_TRA_*`/`WHATSAPP_AVI_*` (patrón per-empresa como SUNAT); sin ellas la marca queda en **mock** (registra en CRM, no manda a Meta); (c) el lead es único por **`(telefono, empresa)`** — un cliente puede escribir a las dos marcas (migración `migrate-crm-whatsapp-2026-07-19.sql`: cambió el UNIQUE global de `telefono`); (d) **idempotencia** del webhook por `message.id` (Meta reintenta); (e) **ventana de 24h**: fuera de ella el texto libre da **409** — solo una **plantilla** reabre (las plantillas proactivas exigen verificación de Meta desde ene-2026; el RUC 10 sin verificar puede recibir+responder en ventana y correr Click-to-WhatsApp, pero NO mandar plantillas en frío); (f) el envío (`src/lib/whatsapp/sender.ts`) **nunca lanza** y detecta el error 131047 (fuera de ventana); (g) `statuses[]` del webhook actualiza `lead_mensajes.estado` (✓/✓✓/leído/⚠) por `whatsapp_message_id`, sin degradar 'leido'. **No necesitas verificar la empresa para OPERAR** el CRM/publicidad; verificar solo escala límites y habilita plantillas. Fuentes (deep research verificado) y runbook Meta: [doc 15 §5](./docs/arquitectura/15-asistente-ia.md) + [historial](./docs/historial-cambios-2026.md).
...
55. **CRM de leads: modo CLARO, sin Kanban y con feedback del bot (20 jul 2026)**: (a) el CRM es **solo modo claro** como el resto de la app — se borraron 256 clases `dark:` (código muerto, `globals.css` fija `color-scheme: light`, gotcha #15); **no re-agregar `dark:` ni superficies `bg-slate-800/900/950`** salvo el velo de los modales (`bg-slate-900/60`, que sí debe ser oscuro). (b) **La vista Kanban se eliminó** (no se usaba); quedan `chat` y `rotacion`. `@hello-pangea/dnd` sigue instalada porque **Despacho** la usa. (c) **Colores de marca**: Transavic **rojo**, Avícola de Tony **ámbar** (igual que sus webs) — estaban invertidos. (d) **Polling con `usePollingVisible`** (`src/lib/use-polling-visible.ts`): se pausa con la pestaña oculta, pero **la carga inicial va aparte y es incondicional** — el helper NO re-ejecuta al cambiar `leadId` (solo si cambia `enabled`), así que todo cambio de conversación necesita su propio `useEffect` de carga inmediata o la vista queda en blanco. (e) **Feedback del bot**: `leads.bot_pensando_desde` (migración `migrate-crm-bot-pensando-2026-07-20.sql`) lo prende el orquestador antes de `callIA()` y lo apaga en un `finally`; la UI solo pinta "escribiendo" si la marca tiene **< 60 s** (un flag colgado se apaga solo). Sirve para que la asesora no conteste encima del bot. Detalle: [historial](./docs/historial-cambios-2026.md).

56. **Reglas del bot de WhatsApp aprendidas a la mala (20 jul 2026)** — cuatro fallas que estaban activas y ya se corrigieron; **respetarlas al tocar `src/lib/chatbot/bot-orchestrator.ts`**: (a) **el `catch` DEBE enviar y persistir** el mensaje de respaldo (`persistirYEnviarBot`) — devolver un string no sirve: el webhook ignora el retorno y el cliente se queda sin respuesta; (b) **el avance del `sequenceIndex` de la rotación es un ÚNICO `UPDATE … jsonb_set … RETURNING`** — con SELECT+UPSERT separados, dos WhatsApp del mismo segundo consumían el mismo índice y ambos leads caían en la misma asesora (el reset diario se decide en JS, gotcha #45c); (c) **nunca enviar la salida del LLM sin sanear**: `sanearRespuestaBot()` en `src/lib/chatbot/sanear-respuesta.ts` descarta vacíos, basura estructural y frases truncadas, y limpia la etiqueta de handoff con flag **global** (sin `/g` un segundo tag llegaba VISIBLE al cliente); (d) el handoff se detecta con `pideHandoff()` (case-insensitive), no con `includes("[HANDOFF]")`. **Backlog priorizado** del estudio del CRM de `conexipema-eventos` (reenganche de clientes fríos, lock+debounce de ráfagas, horario de atención real, Golden Ticket): [historial](./docs/historial-cambios-2026.md).

57. **Cada marca necesita su PROPIA app de Meta y su propio App Secret (20 jul 2026)**: una app de Meta pertenece a UN business portfolio y, para tocar WABAs de otro, exige **Advanced access** de `whatsapp_business_management` (App Review / Tech Provider) — sin él **todo devuelve error 200**. Por eso la WABA del RUC 10 va con una app propia dentro de TONIO LADT, NO con "Transavic CRM". Como Meta firma cada POST con el secret **de la app que lo entrega**, el webhook (que sí es compartido, igual que `META_VERIFY_TOKEN`) valida contra **`META_APP_SECRET` + `META_APP_SECRET_AVI`** y acepta si alguno cuadra (`appSecretsConfigurados`/`firmaValida` en `api/webhooks/meta/route.ts`) — al sumar una marca nueva, agregar su secret ahí, nunca reemplazar. Además: la **rotación de asesoras es por marca** (`porMarca.tra`/`porMarca.avi` dentro de `settings.crm_lead_distribution`; el patrón y la hora de reset siguen compartidos) y la **idempotencia del webhook** ya no es check-then-act sino `ON CONFLICT` sobre el índice único parcial `ux_lead_mensajes_wamid`. Los **textos del CRM se filtran por marca**: plantillas y respuestas rápidas aceptan un campo `empresa` opcional (ausente = las dos) y el chat solo ofrece las de la marca del lead — nunca hardcodear el nombre de una marca en un texto compartido. Correcciones al #52: la verificación del portfolio sube **cupos** (250 → 2 000/24 h, y son **por portfolio**: una marca no hereda el tier de la otra), no habilita "plantillas desde ene-2026"; y la ventana de **72 h del CTWA es de GRATUIDAD** — la de **24 h** sigue decidiendo qué se puede enviar. **Runbook completo del alta de un número (rutas reales de la UI, estado de cada marca, `POST /register`, errores 133005/133016/132001/200, quién hace qué): [doc 28](./docs/arquitectura/28-alta-whatsapp-por-marca.md)** — el alta NO se puede automatizar (Meta lanza reCAPTCHA). Crónica: [historial](./docs/historial-cambios-2026.md).

58. **Migración SUNAT antes del deploy (20 jul 2026):** nunca actives código que lea columnas nuevas antes de aplicar y verificar su SQL en producción con `psql -1 -v ON_ERROR_STOP=1`; hacerlo causó `42703` y dejó la lista sin cargar, aunque los datos seguían intactos. Para reconciliación CPE verifica `15 columnas + 2 claims + 6 índices` y construye desde Git limpio. Un aceptado sin CDR sigue siendo aceptado; si un duplicado aceptado recibe NC, la UI debe mostrar la NC exacta y no ofrecer otra. No implementar un relink puntual de cartera sin procedencia estructurada de anulación y serialización multioperación. Incidente y recuperación: [historial del 20/07](./docs/historial-cambios-2026.md) y [runbook](./docs/arquitectura/24-pruebas-regresion-despliegue.md).

59. **DOS niveles de cuadre de merma — no mezclarlos (30 jul 2026, pedido de Marianela)**: (a) **Cuadre de Pollo** (`/dashboard/cuadre-pollo`, fórmula ÚNICA en **`src/lib/cuadre-pollo.ts`**) cuadra el pollo **globalmente**: `neto comprado − (campo + planta + lo que salió a picar)` vs `aves × 0.32 kg` (parámetro `merma_estandar_ave_kg`; el 0.32 está verificado en las 32 hojas de su Excel — los "0.35 macho / 0.30 hembra" son notas suyas que NO entran en la fórmula). Cuadra si `|diferencia| ≤ 1% del neto`. (b) **Cuadre por Producto** (pestaña de Reportes) solo cubre **`productos.origen_fisico = 'reventa'`** (gallinas, carnes) — comparar compra vs venta por fila para los cortes de pollo es imposible (se compra 1 producto, se venden ~20). ⚠️ **El cuadre usa la SALIDA FÍSICA a corte (3 campos manuales), NUNCA lo facturado por asesoras** — son dos medidas del mismo flujo y sumarlas duplica el 30-45% del día; lo facturado va aparte en "Control de delivery". Columna nueva `productos.origen_fisico` (`vivo`|`desposte`|`reventa`); `rendimiento_porcentaje` NO sirve (está en 100.00 en los 84 productos). Se retiró la "Calculadora Mermas" (`mermas_diarias` tenía **0 filas**); el módulo nuevo escribe esa tabla para que Rentabilidad no pierda el dato. **`salida-carnes` y `cuadre-fisico` ahora usan `fecha_pedido` (entrega), no `created_at`** — los históricos se movieron. (c) **Captura tipo Excel**: los campos que teclea la usuaria aceptan el desglose (`62+62.2+62.5…`, y `70+15+55*7+15` para las aves) vía **`src/lib/expresion-numerica.ts`** — tokenizador propio, **NUNCA `eval`**, y la **precedencia importa** (`70+15+55*7+15`=485, no 3010). El texto se guarda en `cuadre_pollo_dia.expr_*` solo para poder REEDITARLO; **el número es la fuente de verdad** y el POST devuelve 400 si el desglose no evalúa a ese número. (En su Excel el 23% de las celdas de peso son sumas; en CORTE, 29 de 32 días con hasta 11 pesadas.) La pantalla usa el lenguaje visual de hoja de `cuadre-mermas-tab` y la narrativa del Excel (VENTA + MERMA POLLO = DEBIÓ ENTRAR, vs ENTRÓ ⇒ DIFERENCIA). ⚠️ En una hoja así, el `overflow-x-auto` va en el CONTENEDOR y el `minWidth` en el HIJO — juntos empujan el layout de toda la página. (d) **Flexibilidad (ella controla el 100% de su Excel, acá el sistema aporta la mayor parte)**: el GET trae `desglose.{campo,planta,ejecutivas}` por cliente para un acordeón inline — sus queries deben usar **el MISMO WHERE que los totales** o el desglose no suma; **`cuadre_pollo_lineas`** guarda sus líneas propias y su campo **`seccion`** ('entrada'|'salida') es lo que impide sumar al lado equivocado (reemplaza a las columnas fijas de corte, que quedan sin uso); y `cuadre_pollo_dia.kg_campo_ajustado`/`kg_planta_ajustado` permiten corregir un total del sistema con **motivo obligatorio** (CHECK + zod + botón bloqueado; NULL = manda el sistema; el PDF imprime el ajuste, si no deja de ser un control). El POST guarda con **replace-all en `sql.transaction`** e ids por `crypto.randomUUID()` (el batch de Neon no encadena RETURNING). Migraciones `migrate-cuadre-pollo-2026-07-30.sql`, `-expresiones-` y `-lineas-`. Detalle: [doc 09 §7bis](./docs/arquitectura/09-compras-inventario-mermas.md).

60. **Un cron frecuente le cuesta MÁS que todo el tráfico real (31 jul 2026)**: Neon cobra por **CU-hora encendido**, no por consultas, y auto-suspende recién tras **5 min de silencio**. `reconciliar-cpe-sunat` corría **cada 5 min las 24 h** → reseteaba el contador justo antes de que expirara y la base **nunca se dormía**: de una factura de **$22.94**, ~**$19 eran estar prendida sin trabajo** (216 CU-h ≈ 730 h/mes × 0.25 CU). Reglas: (a) **ningún cron de Vercel con cadencia < 15 min sin gate horario** — el gate debe ser **puro, ANTES de tocar la DB** (`dentroDeVentanaEmision()` / `dentroDeVentanaOperativa()` en **`src/lib/ventana-operativa.ts`**, que tiene DOS ventanas independientes: GPS 04:30–22:00 y emisión 05:00–23:00; **no las unifiques**); (b) para Neon **da igual leer que escribir** — cualquier consulta despierta el cómputo, así que optimizar un `UPDATE` a `SELECT` no ahorra nada, lo que ahorra es **no llamar**; (c) al mover el schedule de `repartidores-oscuros`, mover con él `OSCURO_STALE_MIN` (van atados) — pero **NO** el `OSCURO_STALE_MS` de `api/despacho/route.ts`, que es otro knob (pinta el mapa, se mide contra el heartbeat de 90 s). **Supabase se evaluó y NO conviene**: Pro cuesta $25 (más que la factura de hoy), el Free no tiene backups, y migrar son 5-10 días (243 archivos con el driver, 18 `sql.transaction` en caminos de dinero/stock) para conseguir lo mismo — no se usa RLS, ni su Auth, ni Realtime, ni Storage. Detalle: [historial](./docs/historial-cambios-2026.md).

61. **El bot de WhatsApp ("Antonella") vende con la verdad de la DB (5 ago 2026)**: el prompt ya NO está hardcodeado. El conocimiento se arma en `src/lib/chatbot/`: **`carta-comercial.ts`** (curaduría comercial ↔ nombres reales del catálogo; lo que no está ahí NO se cotiza — y la **milanesa está excluida a propósito**), **`config-bot.ts`** (`settings.bot_ventas`: distritos, horarios, mínimos, medios de pago; defaults = los documentos de Antonio), **`contexto-negocio.ts`** (precios reales de `productos` con memo 5 min → *un cambio de precio tarda hasta 5 min en llegar al bot*; hora de Lima; si ya es cliente) y **`prompt-antonella.ts`**. ⚠️ **El prompt vive en CÓDIGO, no en settings**: contiene el token `[HANDOFF]` del que dependen `pideHandoff()` y el apagado de `chatbot_activo` — editable en un textarea, un pegado desprolijo deja al bot sin transferir EN SILENCIO. Al LLM viaja solo el **primer nombre** + nombres de producto (nunca apellido, teléfono, RUC, dirección ni montos). El turno es **ACK-first**: el webhook responde 200 a Meta y el trabajo va en `after()` con **lock** (`leads.bot_lock_token`, TTL 70 s > maxDuration), **debounce 7 s** (`BOT_DEBOUNCE_MS`, junta la ráfaga en UNA respuesta) y descarte pre-envío si entró la asesora (`bot_turno_seq`). **`fallback.ts`**: el cliente nunca queda sin respuesta y nunca se promete una asesora sin notificarla (cascada asesora → candidatos en cola → admin); ⚠️ **un fallback NO apaga el bot** — solo lo apaga un `[HANDOFF]` o una asesora (apagarlo por un 429 pasajero dejaba al lead sin bot para siempre). Lo que llega con el lock tomado no se pierde: `bot_turno_respondido` vs `bot_turno_seq` reencola el turno al soltar. ⚠️ **`leads.nombre` es texto del CLIENTE** (perfil de WhatsApp): pasa por `primerNombre()` con whitelist de letras — sin eso, un lead llamado `[HANDOFF]` apagaba su propio bot. Y `pareceTruncada()` ahora mira la ÚLTIMA LÍNEA: antes descartaba toda lista de precios y hacía imposible cotizar. Migración `migrate-bot-lock-2026-08-05.sql` (psql ANTES del deploy). Pruebas: `npm test` (vitest, funciones puras) + `scripts/simular-conversacion.mjs`. Detalle: [doc 15 §6](./docs/arquitectura/15-asistente-ia.md).

62. **Los leads se reparten POR MARCA y en partes iguales (6 ago 2026)**: `users.empresas TEXT[]` dice qué marcas atiende cada asesora (**NULL o vacío = todas** — ese default evita que un deploy deje a alguien fuera del reparto sin querer). La rotación **y el escalado** filtran por la marca del lead: antes el pool era común y un lead de una marca podía caerle a quien atiende la otra. El modo por defecto es **`equitativo`** (`settings.crm_lead_distribution.modo`): dentro de la marca gana la que MENOS leads recibió hoy **en esa marca**, y desempata la que hace más tiempo que no recibe (`ordenarPorEquidad` en `bot-orchestrator.ts`); los niveles 60/25/15 quedan guardados pero NO se usan (la pantalla de Reparto lo dice y atenúa los porcentajes). Migración `migrate-asesoras-por-marca-2026-08-06.sql`. Ojo: las respuestas rápidas y las plantillas del CRM **ya estaban** separadas por marca (gotcha #57), el reparto era lo único compartido.

63. **El PDF del estado de cuenta avícola muestra el monto POR PRODUCTO (6 ago 2026)**: cada día es un bloque —una fila por producto con su importe + una fila `TOTAL DEL DÍA` que lleva saldo anterior/abonos/saldo actual—, porque antes solo salía la suma y el cliente tenía que sacar calculadora para revisar el cobro. Reglas al tocar `pdf-estado-cuenta-avicola.ts`: **NUNCA usar `rowSpan`** (jspdf-autotable parte mal esos bloques entre páginas; se usan filas planas + `rowPageBreak: "avoid"`); si los importes no suman el total, la diferencia sale como fila **"Ajuste"** en vez de dejar un PDF que se contradice (hoy no pasa: el API calcula el total sumando los ítems); y la construcción de filas vive en **`construirFilasEstadoCuenta`**, pura y con tests. `vitest` ya resuelve el alias `@/`.

64. **Un gasto se registra con LA FECHA EN QUE SE GASTÓ (6 ago 2026)**: el formulario vivía solo dentro de una caja abierta y mandaba `new Date().toISOString()` (siempre hoy, y en UTC → después de las 19:00 Lima guardaba MAÑANA); para cargar el mes atrasado hubo que abrir una caja falsa de S/10,000 y la fecha real terminaba escrita dentro de la descripción ("PETER CON 13-07"). Ahora el formulario es **compartido** (`src/app/dashboard/gastos/form-gasto.tsx`, usado por Gastos y por Caja), en **Gastos se registra SIN caja abierta**, la fecha va primera y **no se resetea al guardar** (cargar 20 gastos del mismo día = 20 montos). ⚠️ **REGLA GENERAL: todo movimiento de dinero con fecha elegible debe escribir en `transacciones` las DOS columnas, `fecha` y un `created_at` SINTÉTICO en ese día** — el arqueo filtra por `created_at >= caja.abierta_at`, así que sin eso un pago retroactivo descuenta de la caja de HOY (esa fuga estaba viva en `lib/proveedores/pagos.ts` y se corrigió). Validación en **`src/lib/gastos/fecha-gasto.ts`** (pura, con tests): futura NO, hacia atrás SIN límite, "hoy" resuelto por SQL. `PATCH`/`DELETE /api/gastos/[id]` permiten corregir (antes un gasto mal cargado era permanente) y, como el alta, dan **409 si la caja de esa fecha ya fue arqueada**. Migración `migrate-gastos-fecha-2026-08-06.sql`; reubicar lo ya cargado con `scripts/corregir-fechas-gastos.mjs` (dry-run → revisar → `--apply`) y **alinear los pagos a proveedor** con `scripts/alinear-created-at-pagos-proveedor.mjs` (idem; solo mueve `created_at` al día de la `fecha`, jamás al revés — hay 6 movimientos del POS con el desfase invertido, que son otro bug: ahí la `fecha` quedó en UTC). En producción ya se aplicaron los dos (14 gastos y 167 pagos, 6 ago 2026). Detalle: [doc 10 §8](./docs/arquitectura/10-pos-caja-tesoreria.md).

65. **Las listas de opciones se amplían DESDE DONDE SE USAN (6 ago 2026, pedido de Marianela: "Delivery")**: `categorias_gasto` y `tipos_doc_compra` viven en `settings.parametros_negocio` y solo se editaban en `/dashboard/configuracion` — admin-only y en otra pantalla, así que cuando faltaba una opción la vía real era abandonar lo que estabas haciendo. Ahora el `<select>` del gasto y el del tipo de documento de compra traen **"➕ Nueva opción…"** (patrón centinela `__nueva__`, el mismo que ya usaban 3 selects de categorías de producto). Reglas: (a) el alta va por **`POST /api/parametros-negocio/lista`** (admin **+ produccion**, los mismos que registran el movimiento) que hace el **read-modify-write en el SERVIDOR** — NUNCA mandar el objeto entero desde el cliente, porque `POST /api/settings` **reescribe la clave completa** y pisaría los umbrales que otro admin esté editando; por lo mismo `configuracion-client.tsx` re-lee antes de guardar y conserva las opciones que aparecieron mientras su pantalla estaba abierta; (b) la lógica pura vive en **`agregarALista`** (`src/lib/parametros-negocio.ts`, con tests): recorta y colapsa espacios, capitaliza la inicial, deduplica **ignorando tildes y mayúsculas** y, si ya existe, **NO es error** — devuelve la que hay y la UI la selecciona; (c) **quitar o renombrar sigue siendo admin, en Configuración** (afecta el catálogo entero), y el formulario solo muestra un enlace discreto para que se descubra dónde está; (d) el `<select>` del gasto NUNCA debe arrancar con la lista vacía o el centinela se muestra como si fuera la categoría elegida — por eso los clientes inicializan con `PARAMETROS_NEGOCIO_DEFAULT`. Sin migración.

66. **`/api/productos` devuelve `{ data }`, NO un array — y la guía de una venta de planta (7 ago 2026, Fase 1 de las peticiones de Ariana)**: (a) `ventas-planta-client.tsx` chequeaba `Array.isArray(json)` sobre el envoltorio → el catálogo quedaba `[]` **en silencio** (el `.catch(()=>{})` tapaba todo) y el buscador del modal *Editar venta* **no encontraba NINGÚN producto** (no era que faltara "molleja"). Comprobar siempre `Array.isArray(json?.data)`, como ya hacen `ProductSelector.tsx:66` y `emitir-client.tsx:784`. (b) La orden imprimible **`/pedidos/[id]/guia` ya sirve para pedidos POS** (`origen='pos_planta'`) y **no restringe por estado** — solo faltaban enlaces; ahora *Ventas de Planta* tiene **Imprimir** y **Enviar guía**, también en anuladas (el ticket lleva banda ANULADA). (c) Piezas nuevas: `lib/planta/guia-pos.ts` (`guiaDeVentaPlanta`, espejo de `lib/avicola/guia.ts`), `GET /api/pos/ventas/[id]`, `ticket-guia-planta.tsx`, `guia-planta-modal.tsx`. **Diferencias con campo que NO se deben copiar tal cual:** las líneas llevan **unidad variable** (kg|uni), el **estado de cuenta es OPCIONAL** (solo en crédito) y los abonos de planta cuelgan de la **cobranza**, no del cliente. (d) **WhatsApp = compartir + descargar + `wa.me`**, NO Cloud API (la ventana de 24 h de Meta la haría fallar: estos clientes no escriben al número del CRM); ⚠️ un `wa.me` **no puede adjuntar la imagen**, por eso "Abrir chat" descarga primero. Detalle: [historial](./docs/historial-cambios-2026.md).

67. **La ficha del cliente de PLANTA muestra el movimiento, y el vínculo pedido↔cliente ya se persiste (7 ago 2026, Fase 2 de las peticiones de Ariana)**: (a) **`pedidos.cliente_planta_id`** (migración `migrate-planta-historial-2026-08-07.sql`) cierra el hueco estructural — antes el POS insertaba `cliente_id = NULL` y el ÚNICO puente era `cobranzas_planta.pedido_id`, que **solo existe a crédito**, así que una compra al CONTADO era invisible desde la ficha y ni contaba como "última compra". Hay **DOS puntos de escritura obligatorios**: el INSERT de `api/pos/route.ts` y la **sentencia G** del PATCH de `api/pos/ventas/[id]` — con solo el primero, editar una venta cambiándole el cliente dejaría el id viejo pegado mientras `razon_social`/`ruc_dni` sí se actualizan. Backfill: crédito desde `cobranzas_planta` (100%) + contado por `ruc_dni` (determinista por `ux_clientes_planta_ruc`). (b) ⚠️ **DOS AGREGADOS QUE NO SON EL MISMO NÚMERO**: el **saldo** lo mueven `clientes_planta.saldo_anterior` (columna nueva, DEFAULT 0) + las ventas a **CRÉDITO** + los abonos; lo **comprado** incluye además el **CONTADO**, que no genera deuda. Mezclarlos hace que la ficha se contradiga sola — por eso el contado va SIEMPRE aparte (`total_contado`). (c) Piezas nuevas espejo de campo: `lib/planta/historial.ts`, `lib/planta/estado-cuenta.ts` (**pura**, fuente única de pantalla y PDF, con tests), `lib/reportes/pdf-estado-cuenta-planta.ts` y `estado-cuenta-planta-modal.tsx`. **NO copiar la query de campo tal cual**: en planta el abono cuelga de la **COBRANZA** (`abonos_planta.cobranza_id`), no del cliente; la venta vive en `pedidos` (`origen='pos_planta'`); los roles son `admin`+`produccion` (campo es `admin` a secas); y las líneas llevan **unidad variable** (kg|uni). (d) **`OrdenImprimible.estadoCuenta` sigue declarada y SIN renderizar a propósito** — se quitó en `7e4f3c5` (19 jul); el saldo viaja por la guía JPEG de WhatsApp y por este PDF, no por la orden impresa. Detalle: [historial](./docs/historial-cambios-2026.md).

68. **Producción puede ver otras fechas y los ya despachados, solo para REIMPRIMIR (7 ago 2026, Fase 3 de las peticiones de Ariana)**: el bloqueo nunca estuvo en el documento — **`/pedidos/[id]/guia` no lee `p.estado`** y sirve cualquier pedido de cualquier fecha; lo que lo escondía era la navegación (la query filtraba los 3 estados de producción, y el único enlace vivía DENTRO del modal de pesos). Ahora hay selector de fecha (el endpoint **ya aceptaba `?fecha=`** y nadie se la mandaba), toggle **`?incluir_despachados=1`** y botón **Imprimir orden** en la tarjeta. **El default NO cambia** (cola del día) y un despachado no abre el modal: `/pesos`, `/listo` y `/reabrir` devuelven 400 fuera de los 3 estados. ⚠️ **Tres trampas al ampliar el filtro de estados:** (a) las ventas del POS nacen en **`Entregado`** → sin `COALESCE(p.origen,'asesor') <> 'pos_planta'` se cuelan en la cola de Producción; (b) **`estadoBadge` no tenía `default`** → `undefined` y la tarjeta reventaba al leer `badge.color`; (c) el **backfill lazy de ítems escribe en DB durante un GET** (gotcha #31) → se acotó a los 3 estados de producción para no escribir sobre cientos de pedidos cerrados al listar fechas pasadas. Detalle: [historial](./docs/historial-cambios-2026.md).

69. **Alertas de arribo con ETA honesto (11 ago 2026)**: "por llegar"/"llegado" se deciden en **`src/lib/eta-reparto.ts`** (puro, con tests) — calibración por-viaje del Directions de iniciar-viaje (`pedidos.eta_factor_ruta`/`eta_velocidad_kmh`, migración `migrate-eta-honesto.sql`, NULL=defaults), mensaje con minutos REALES (2..5; <2 se suprime; viaje ≤6 min nace suprimido), exclusión mutua (llegado consume ambos flags), **claim atómico ANTES de notificar** (`UPDATE … WHERE flag=FALSE AND estado='En_Camino' RETURNING`), ping con `capturedAt` a >120 s NO evalúa alertas, y el UPSERT de `rider_locations` no retrocede `captured_at`. El "hace X" de la UI vive en **`src/lib/tiempo-relativo.ts`** (SIEMPRE floor — no duplicar helpers). Reglas: [doc 16 §1.1](./docs/arquitectura/16-notificaciones-cron.md); crónica: [historial](./docs/historial-cambios-2026.md).

70. **`maxDuration` es OBLIGATORIO en toda ruta que firme o hable con SUNAT (11 ago 2026)**: sin él Vercel mata la función a los ~15 s **después** de que la fila quedó reservada y **antes del `catch`** → falla sin dejar rastro. Le pasó al cron `resumen-diario-sunat`: **60 corridas, todas colgadas en `enviando`, sin ticket ni mensaje, durante 70 días** — y como recorría las 2 empresas en serie, **avicola casi nunca corría**. Reglas: (a) `export const maxDuration = 60` (como `comprobantes/emitir`, `[id]/reintentar`, `guias/emitir`); (b) un cron multi-empresa lleva **try/catch por empresa**; (c) toda fila de "en curso" necesita un saneo que la cierre con causa (`sanearResumenesColgados()`). **El RC quedó en modo SIMULACIÓN** (`dryRun` por defecto; se enciende con `SUNAT_RESUMEN_DIARIO_AUTO="true"`): las 579 boletas aceptadas **ya tienen CDR** de `sendBill`, así que declararlas en un RC sería declaración repetida — confirmar con el contador antes de encenderlo. `ESTADOS_BOLETA_EN_RESUMEN` (en `lib/sunat/resumen-diario.ts`) es fuente única de qué boleta entra: **nunca una incierta** (`por_confirmar`/`emitiendo`/`error`/`no_registrado`). Detalle: [historial](./docs/historial-cambios-2026.md).

71. **Consulta Integrada de boletas: AVI ✅ operativa, TRA pendiente — y el canal individual caduca a los 5 días (19-20 ago 2026)**: `SUNAT_AVI_CONSULTA_CLIENT_ID/SECRET` ya existen (app SOL "AVICOLA ERP CONSULTA CPE" del RUC 10, en Vercel + `.env.local`; runbook con las trampas del portal en [doc 11](./docs/arquitectura/11-comprobantes-sunat.md) — ⚠️ NO confundir con "Gestión Credenciales de API SUNAT" del menú raíz, que es donde vive la app de GRE y no se toca). Con la llave puesta, las 9 atascadas se resolvieron en una noche: **5 estaban ACEPTADAS** (las fallas 0130/0133 registran "a medias" — 225/226/230/233 con reemplazo aceptado = **dobles/triple emisión** → cuadro para el contador/NC, no emitir nada más en esos pedidos; 284 limpia) y **4 NO EXISTEN** (274/315/318/319 → `no_registrado`). Regla nueva: **`sendBill` de una boleta solo se acepta hasta 5 días después del IssueDate** — más tarde SUNAT rechaza con *"Solo puede enviar el comprobante en un resumen diario — Presentación fuera de fecha (5 días)"* (probado con la 315) → el "Reintentar mismo número" solo sirve si se destraba a tiempo; una no-registrada vieja se resuelve con **boleta NUEVA de fecha actual** (ojo si cruza de mes tributario: contador). Con la credencial activa, la reconciliación resuelve en minutos/horas, así que el limbo de semanas no se repite **en Avícola**; las boletas B001 de **Transavic chocarían con el mismo muro** hasta crear `SUNAT_TRA_CONSULTA_*` (misma receta, clave SOL RUC 20). Detalle: [historial](./docs/historial-cambios-2026.md).

69. **Un lead sin reclamar se lo queda la ASESORA de turno, no el admin (13 ago 2026)**: el reparto por marca (#62) funcionaba, pero **ningún lead llegaba a las asesoras**. Un lead nace `en_cola` con `vendedor_id = NULL` y solo `candidato_actual`; si nadie toca "Atender" en **15+45+60 s**, `escalateLead` lo asignaba al primer admin (`SELECT id FROM users WHERE role='admin' LIMIT 1`). Como nadie alcanza a reclamar en 2 minutos, **TODOS terminaban ahí**: en producción Mateo tenía 10 leads y las 2 asesoras de Avícola CERO. Peor, se realimentaba: al escalar se pone `candidato_actual = NULL` y la equidad cuenta por `COALESCE(vendedor_id, candidato_actual)`, así que el lead dejaba de contar para nadie, las candidatas quedaban empatadas en 0 **para siempre** y el desempate alfabético le ofrecía **siempre a la misma**. Ahora el fallback final llama a `rotateAndSelectCandidate` y **asigna a una asesora real de esa marca**; el admin queda solo como red de seguridad para cuando la marca **no tiene ninguna asesora habilitada** (caso de configuración, con aviso explícito). La regla de equidad vive en **`src/lib/chatbot/equidad-leads.ts`** (pura y con tests) — se sacó de `bot-orchestrator.ts` porque ese importa `neon` en el top level y eso mata cualquier test (gotcha #13). ⚠️ Que el desempate **alfabético** se use seguido es señal de que algo está roto: significa que las candidatas están todas en cero y sin historial. Data-op del arrastre: `scripts/reasignar-leads-mateo-2026-08-13.sql`. Detalle: [historial](./docs/historial-cambios-2026.md).

70. **El bot CUBRE EL HUECO: solo habla cuando no hay asesoras (13 ago 2026)**: hasta ahora Antonella respondía las **24 h**, encima de las asesoras. Ahora `settings.bot_ventas.cuando_responde` decide: **`"fuera_horario"` (DEFAULT)** = calla dentro del horario de atención y atiende de noche y días no laborables; `"siempre"` = comportamiento viejo. La regla es **pura y con tests**: `botDebeResponder({cfg, dentroDeAtencion})` en `src/lib/chatbot/fallback.ts`; el gate está en `ejecutarTurno` (bot-orchestrator, donde antes solo se miraba `cfg.activo`). ⚠️ **Cambiar el default cambia producción al instante**, porque `settings.bot_ventas` **no existía** y todo corría con los defaults del código. (b) **El `[HANDOFF]` nocturno dejaba al cliente sin bot Y sin humana** —se apagaba `chatbot_activo` y nadie entraba hasta la mañana—: ahora, fuera de horario, antes de apagar se envía `mensajeCierreFueraDeHorario()` diciendo cuándo lo retoman. (c) El prompt fuera de horario pasó de pasivo ("no prometas una llamada inmediata") a ORDENARLE que le diga al cliente el horario. (d) **`WelcomeBotConfig.tsx` ERA UI MUERTA**: guardaba en `settings.crm_welcome_bot`, que ningún proceso leía — mostraba "Horario Comercial" y "Mensaje Fuera de Horario" y no hacía nada, y por eso parecía que había DOS bots. Se reemplazó por **`BotVentasConfig.tsx`**, que edita la config REAL; la clave `crm_welcome_bot` se sacó de la lista blanca de `/api/settings`. ⚠️ Ese panel hace **merge sobre lo que ya había** (`{...crudo, ...cfg}`) porque `POST /api/settings` **reescribe la clave completa**: mandar solo los campos del formulario borraría distritos, mínimos y beneficios. Detalle: [historial](./docs/historial-cambios-2026.md).

---

## 13. Estado del proyecto (resumen — crónicas completas en [docs/historial-cambios-2026.md](./docs/historial-cambios-2026.md))

> **Correcciones vigentes frente a notas históricas de esta sección:** el CDR se
> descarga como **ZIP crudo de SUNAT**, no como XML extraído; RawBT ya está en el
> código desplegado y solo espera validación con la impresora real. El lote operativo
> del 13 jul ya fue probado en `dev-hugo`, pero no se ha migrado ni desplegado en
> producción. Cualquier viñeta antigua que diga lo contrario describe una etapa ya
> superada.
...
0. **Lee primero [`docs/arquitectura/README.md`](./docs/arquitectura/README.md)** — tiene un mapa "si vas a tocar X, lee Y" que te ahorra tiempo. Los 27 documentos temáticos tienen verificación contra código real.
...
2. **Si vas a agregar una nueva tabla o columna**, crea `scripts/migrate-<feature>.sql` y su `rollback-<feature>.sql`, idempotentes; prueba por `psql` en `dev-hugo`. NO modifiques migraciones existentes ni el `seed.mjs`.
...
### Dominios y correo POR MARCA (19 jul 2026)
...
- **Directrices de UX/UI Críticas**:
  - **Evitar Scroll en Inputs Numéricos (`type="number"`):** ya hay un **guard GLOBAL** en `DashboardLayout.tsx` (listener `wheel` passive que hace `blur()` del `input[type=number]` enfocado) — cubre los ~62 inputs del sistema y los nuevos, **no hace falta poner `onWheel` en cada uno**. Los `onWheel` sueltos que existen no estorban.
  - **Separador de miles (30 jul 2026):** la app formatea con `toLocaleString("es-PE")`, o sea `2450 → "2,450.00"`. Cualquier campo que PARSEE texto numérico tecleado debe resolver la coma por forma (coma + 3 dígitos = miles; coma + 1-2 dígitos al final = decimal) — ver `src/lib/expresion-numerica.ts`. Leer la coma como decimal a secas convierte `2,450` en **2.45**: un error de 1000× silencioso que la propia pantalla induce.
  ...
  - **Tonos fuera de la escala de Tailwind = elemento INVISIBLE (20 ago 2026):** la paleta v4 va 50,100,…,600,700,…,950. Un `bg-emerald-650` (o `-150`) **no genera CSS y Tailwind no avisa**; si el elemento además lleva `text-white`, queda blanco sobre blanco. Le pasó al botón principal de `ArriboPopup`: la asesora veía "un campo de texto vacío" y usaba la ✕. Al inventar un tono hay que definirlo en el `@theme` de `globals.css` (hoy solo tiene background/foreground/fuentes) o usar uno de la escala. Quedan `border-gray-150`, `text-red-650`, `text-slate-650`, `text-gray-650` sueltos (solo bordes/texto, no rompen legibilidad).

72. **Un aviso de arribo interrumpe solo mientras está pasando (20 ago 2026)**: a una asesora le saltaban avisos de "motorizado en destino" de pedidos entregados **ayer o antes**, repetidos. No se re-creaban (el productor filtra `En_Camino` + fecha de hoy + claim atómico + ping ≤120 s): una notificación **no leída no caduca nunca** y `ArriboPopup` resucitaba el backlog en cada sesión, porque su anti-repetición vive en `sessionStorage` (muere al cerrar pestaña; en celular cada apertura desde WhatsApp es sesión nueva) y tomaba `alertasArribo[0]` cada 15 s → desfile. Reglas: (a) **`cerrarAlertasArriboDePedido()`** (`lib/notificaciones.ts`) marca leídas las alertas del pedido en las **CINCO** vías que lo sacan de `En_Camino` (entregar/fallar, revertir, cancelar viaje, reprogramar y el **PATCH genérico `/api/pedidos/[id]`** = "Entregar" de la lista + "desasignar" de Despacho, el camino más usado y el que se olvidó en la v1 del fix) — si agregas otra, llámala o el backlog se reacumula; (b) **`avisoArriboVigente()`** (`lib/eta-reparto.ts`, puro con tests) limita el popup a la última hora — `pedido_reprogramado` NO caduca así (es agenda); (c) la ✕/Escape marcan leído el arribo (antes solo el botón gris, y esa asimetría dejaba la alerta viva para siempre); (d) el popup muestra la antigüedad. Backlog: `scripts/cerrar-alertas-arribo-huerfanas-2026-08-20.sql`. Detalle: [doc 16 §3.1](./docs/arquitectura/16-notificaciones-cron.md).

73. **Permiso POR USUARIO para cambiar el precio de venta (20 ago 2026, pedido de Antonio para Yali)**: `users.puede_editar_precio_venta BOOLEAN` deja que **una asesora** edite `productos.precio_venta` desde el catálogo. Se prende en el modal de usuarios (admin-only, solo visible con rol `asesor`) y **viaja en el JWT ⇒ aplica al próximo login**, igual que `solo_lectura` (#51). Reglas: (a) el permiso es **POR CAMPO, no por endpoint** — `PATCH /api/productos/[id]` acepta 8 campos en el mismo body, así que el guard vive en **`src/lib/permisos-precios.ts:evaluarEdicionCatalogo()`** (puro, con tests): admin = todo; asesora con bandera = SOLO `precio_venta`; cualquier otro campo → 403 nombrándolo (fail-closed). **Si agregas un permiso puntual nuevo, extiende ese helper, no el guard del rol**; (b) la respuesta del PATCH devuelve `precio_compra: null` a quien no es admin — sin eso el costo se filtra por la pestaña de red aunque la tabla no lo pinte; (c) en la UI `isAdmin` (costo, margen, historial, alta y baja) quedó **separado** de `puedeEditarPrecioVenta` (solo la celda de precio de venta, escritorio y móvil); (d) ⚠️ **el precio de catálogo ES el piso de venta** de las asesoras (`lib/autorizaciones-precio.ts:85-91`): no existe `precio_minimo`, así que quien lo cambia mueve el mínimo de todo el equipo — está advertido en el confirm y en el modal. De paso se cerró que **`GET /api/productos` no exigía sesión** (el catálogo con precios se respondía sin login). Migración: `scripts/migrate-permiso-precio-venta-2026-08-20.sql`. Detalle: [historial](./docs/historial-cambios-2026.md).

74. **Anular un pago DEBE soltar lo que referencia a la deuda (21 ago 2026, caso de Marianela)**: `anularPagoProveedor` marcaba el pago `anulado`, hacía el contraasiento y recalculaba `cuentas_por_pagar.monto_pagado` filtrando `p.estado='registrado'` (→ **0**), pero **no borraba `pagos_proveedores_aplicaciones` ni limpiaba `pagos_proveedores.deuda_prioritaria_id`**. Ambas apuntan a `cuentas_por_pagar` con **FK `ON DELETE RESTRICT`**, así que después era IMPOSIBLE anular la compra: la guarda miraba el **caché** (0 → deja pasar) y el `DELETE` chocaba con la **tabla real** (error `23503`). ⚠️ **Regla: una guarda no puede medir un caché cuando lo que restringe es una FK sobre la tabla física** — por eso `compras/[id]/anular` ahora pregunta también por filas de pagos VIVOS (`EXISTS … JOIN pagos_proveedores WHERE estado='registrado'`) y limpia el rastro de los anulados antes del DELETE. La anulación de pago borra sus aplicaciones **después** del contraasiento (que usa `deuda_prioritaria_id` como referencia) y **antes** del recálculo. Se pierde a propósito el desglose "a qué documentos se aplicó" de un pago anulado (decisión de Hugo); pago, monto, autor, fecha, motivo y contraasiento se conservan. Y **`error.message` de Postgres JAMÁS viaja al cliente**: `src/lib/errores-sql.ts` (puro, con tests) lo traduce en los 4 caminos del módulo (anular/editar compra, editar/borrar deuda manual). Arrastre histórico: `scripts/limpiar-aplicaciones-pagos-anulados-2026-08-21.sql`. Detalle: [historial](./docs/historial-cambios-2026.md).
